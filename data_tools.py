...
def getCut(cut_str, y, p, recoE, key):

    # No Cut
    if cut_str == 'No Cut':
        return np.array([True for _ in recoE[key]]), y['energy'][getRecoFilter(p[key]['reco'], y) * y['passed_STA5']] if p[key]['sta5'] else y['energy'][getRecoFilter(p[key]['reco'], y)]

    # Quality Cut
    if cut_str == 'Quality Cut':
        return y['quality_cut'][getRecoFilter(p[key]['reco'], y) * y['passed_STA5']] if p[key]['sta5'] else y['quality_cut'][getRecoFilter(p[key]['reco'], y)], y['energy'][y['quality_cut']]
...
def load_data(fileList, infill=False):
    ...
    domKeys = ['charge_HLC', 'charge_SLC', 'time_HLC', 'time_SLC'] # Keys tied to DOMs
...
def dict2mat(d):
    ...
    for i, charges in enumerate(d['charge_HLC']):
        for dom, charge in charges.items():
            ...
            if dom[2:] in tank2s:
                data[i][y_i][x_i][2] += modLog(charge)
    ...
    for i, charges in enumerate(d['charge_SLC']):
        for dom, charge in charges.items():
            ...
            if dom[2:] in tank2s:
                data[i][y_i][x_i][3] += modLog(charge)
...
def load_preprocessed(filePath, mode, comp=['p','f']):
...
"""
def dataPrep(x, y, clc=True, sta5=False, q=None, t=None, t_shift=False, normed=False, reco=None, cosz=False):
    ...
    # Time Shift
    if t_shift and t != False:
        # Find nonzero minimum across all time layers
        t_nonzero_min = np.min(out_array[...,idx:][np.nonzero(out_array[...,idx:])])
        # Subtract nonzero mimimum from all nonzero values across all time layers
        out_array[...,idx:][out_array[...,idx:] != 0] -= t_nonzero_min
