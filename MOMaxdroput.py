...
Mpool_layer = layers.MaxPooling2D(pool_size=(2, 2), strides=None, padding="valid", data_format=None)(conv2_layer)
conv3_layer = layers.Conv2D(16, kernel_size=3, padding='same',activation="relu")(Mpool_layer)
...
drop1_layer = layers.Dropout(.4,)(dense1_layer)
dense2_layer = layers.Dense(256,activation='relu')(drop1_layer)
...
history = model.fit(
    {"input1_1":x_i[0],"input_2":x_i[1].reshape(-1,1)}, temp_y, epochs=numepochs,validation_split=0.15,callbacks=callbacks)


# In[ ]:


np.save("MOMaxdropout.npy",prep)
