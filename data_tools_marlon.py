...
"""
def dataPrep(x, y, q=None, t=None, normed=False, reco=None, cosz=False, t_norm=None):
             #recocut=False):
    ...
    #Norm time data using minmaxscalar  
        if t_norm != None:
            emit = np.array(out_array[...,-2:])
            time_n = emit.reshape(10995460,10,)

            scaler = MinMaxScaler()
            model = scaler.fit(time_n)
            out_array[...,-2:]=model.transform(time_n)
