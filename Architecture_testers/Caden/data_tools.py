...
"""
def dataPrep(x, y, q=None, t=None, normed=False, reco=None, cosz=False, t_shift = None): #[zeroshift, datashift]
             #recocut=False):
    ...
    for k, mergeType in qtDict.items():
        ...
        if mergeType == False: #says we dont want that data at all
            new_dim -= 2
        # None indicates do nothing to the layers, everything else merges
        elif mergeType != None: #says we want to combine layers somehow
            new_dim -= 1
    ...
    for k, mergeType in qtDict.items(): #question, where do we ever edit y??
        ...
        if t_shift != None: #question, am i just modifying x, not y?
            z = t_shift[0]#shift amount for 0's
            d = t_shift[1]#shift amount for data
            shift = lambda a: np.where(a == 0, a-1, (a - minimum))
            for idx, event in enumerate(out_array):
                minimum =  np.min(event[np.nonzero(event)])
                out_array[ idx,...,2:] = shift(out_array[idx,...,2:], minimum)
